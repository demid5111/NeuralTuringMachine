...
def run_eval(sess, model, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, data_generator, args, target_point, labels, outputs, inputs, batches, store_heat_maps=False, generalization_num=None):
    task_loss = 0
    task_error = 0
...
def eval_performance(sess, data_generator, args, model, target_point, labels, outputs, inputs, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, curriculum_point, store_heat_maps=False):
    # target task
    ...
    multi_task_loss, multi_task_error = run_eval(sess, model, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, data_generator, args, target_point, labels, outputs, inputs, batches)
    ...
    if curriculum_point is not None:
        ...
        curriculum_point_loss, curriculum_point_error = run_eval(sess, model, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, data_generator, args, target_point, labels, outputs, inputs, batches)
    else:
...
def eval_generalization(sess, model, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, data_generator, args, target_point, labels, outputs, inputs):
    res = []
    seq_lens = []
    if args.task == 'copy':
    ...
    for i in seq_lens:
        ...
        loss, error = run_eval(sess, model, inputs_placeholder, outputs_placeholder, max_seq_len_placeholder, data_generator, args, target_point, labels, outputs, inputs, batches, store_heat_maps=args.verbose, generalization_num=i)
        res.append(error)
    return res
