...
def create_chart(series, ticks, granularity=1000, type='error', bits_per_number=4):
    ...
    plt.close()
    plt.plot('x', 'data', data=filtered_data_df, marker='o', color='black',
...
def save_chart(plot, type):
    ...
    path_template = tmp_artifacts_root / f'{type}.eps'
...
def main(args):
    ...
    plot = create_chart(series=errors, ticks=steps, granularity=5000, type='error', bits_per_number=4)
    save_chart(plot, type='error')
    plot = create_chart(series=losses, ticks=steps, granularity=5000, type='loss', bits_per_number=4)
    save_chart(plot, type='loss')
